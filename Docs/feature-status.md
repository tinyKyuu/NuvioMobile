...
| Ordered add-on and catalog recovery after connectivity returns, with a bounded per-profile manifest cache | Fork state machine and cache design, with delayed-state presentation adapted from official Nuvio commit [`085e8dc6`](https://github.com/NuvioMedia/NuvioMobile/commit/085e8dc6aaf5072541130be852a782998fc3dbad) | iOS, iPadOS, and Android shared source; Android recovery flow tested | In testing | Adapted; official upstream has delayed loading/error states but no equivalent central recovery coordinator or bounded, versioned per-profile manifest cache at the reviewed tip | [F07 review evidence](../F07-REVIEW.md); implementation commit `7a833999`; PR pending; 997 Android-host tests, Android debug assembly, Kotlin/Native test compilation, and Xcode simulator build passed; Android emulator exercised offline-to-online recovery with one healthy and one unavailable add-on; iPhone and iPad simulators covered cold launch/layout only; physical iPad recovery remains required before release |
