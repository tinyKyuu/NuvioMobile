...
| Bounded manual reconnect sessions, measured root-header controls, a measured tablet dock, and transient offline simulation | Fork follow-up to F07 | iOS, iPadOS, and Android shared source | In testing | No equivalent found at the reviewed base | [PR #32](https://github.com/tinyKyuu/NuvioMobile/pull/32); one press probes immediately, makes at most three attempts with 2-second and 4-second backoff, and stops within 15 seconds. Repeated presses and path events coalesce, backgrounding or profile replacement cancels the session, and local-only Home remains until catalog recovery completes. Reconnect stays trailing, reserves stable widths across Reconnect, Restoring, and failure states, uses a low-prominence idle treatment, and strengthens its pressed and restoring feedback. The same control is available on offline Details and runs the shared metadata/catalog recovery pipeline. The tablet dock remains one pill-shaped surface at every width: destinations share one selected indicator, labels sit below icons in the full presentation, Settings drops its label first, and all labels disappear only when the measured width requires it. Root navigation now yields its space while the software keyboard or download selection is active. Advanced settings can simulate offline mode without changing device Wi-Fi; Reconnect clears the transient simulation before normal recovery. The latest complete 1,145-test Android-host suite, both Android debug variants, Kotlin/Native iOS simulator test compilation, unsigned simulator build, and signed physical-iPad build passed. The refreshed build was installed in place and launched without clearing app data; interactive acceptance remains pending. |
| Adaptive Library title views, watched filtering, independent download aggregation, and explicit offline availability | Fork follow-up to F07 and the completed-download Library | iOS, iPadOS, and Android shared source | In testing | No equivalent found at the reviewed base | [PR #32](https://github.com/tinyKyuu/NuvioMobile/pull/32); primary views are All titles, Downloads, and conditional Cloud files. All titles is a deduplicated union of explicit local membership, the selected connected Trakt or Simkl library, and playable downloads; downloading does not change local or tracking membership, while deleting an unsaved download removes it from All titles. The explicit Watch status filter precedes the selected sort, and partly watched series remain Unwatched until existing completion state marks the series complete. Offline title cards distinguish downloaded media from internet-required entries, and internet-required cards stay in Library with an explanation instead of opening blank details. Downloaded, internet-required, watched, overflow-menu, and active-selection poster overlays use one shared badge component with regular and large width-driven sizes; accent badges no longer add a dark outline, and large vertical Library cards add concise media-type and year metadata. Episode artwork groups watched and availability states on one compact status rail. Offline series Details offers a session-local `Availability: All / Downloaded` selector when filtering can remove unavailable episodes and empty seasons; its animated season snapshot no longer crashes when a filtered-out season is leaving the composition. Download selection uses `Select`, a clear close action, neutral `Deselect all`, and crimson removal; a fully successful removal exits selection, while partial failures remain selected for recovery. The Downloads page combines current transfers with a collapsible policy card and a route back to completed Library downloads. Compact and stacked presentations, canonical deduplication, provider visibility, watched projection, offline navigation, season-filter transitions, and selection state are covered by host tests. The latest complete 1,145-test Android-host suite, both Android debug variants, Kotlin/Native iOS simulator test compilation, unsigned simulator build, and signed physical-iPad build passed. The refreshed build was installed in place and launched without clearing app data; interactive acceptance remains pending. |
