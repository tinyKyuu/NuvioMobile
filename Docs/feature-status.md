...
| Bounded manual reconnect sessions, measured root-header controls, a measured tablet dock, and transient offline simulation | Fork follow-up to F07 | iOS, iPadOS, and Android shared source | In testing | No equivalent found at the reviewed base | [PR #32](https://github.com/tinyKyuu/NuvioMobile/pull/32); one press probes immediately, makes at most three attempts with 2-second and 4-second backoff, and stops within 15 seconds. Repeated presses and path events coalesce, backgrounding or profile replacement cancels the session, and local-only Home remains until catalog recovery completes. Reconnect stays trailing, reserves stable widths across Reconnect, Restoring, and failure states, uses a low-prominence idle treatment, and strengthens its pressed and restoring feedback. The tablet dock remains one pill-shaped surface at every width: destinations share one selected indicator, labels sit below icons in the full presentation, Settings drops its label first, and all labels disappear only when the measured width requires it. Root navigation now yields its space while the software keyboard or Download manager is active. Advanced settings can simulate offline mode without changing device Wi-Fi; Reconnect clears the transient simulation before normal recovery. The latest complete 1,143-test Android-host suite, both Android debug variants, Kotlin/Native iOS simulator test compilation, unsigned simulator build, and signed physical-iPad build passed. The build was installed in place and launched without clearing app data; interactive acceptance of the final layout remains pending. |
| Adaptive Library title views, watched filtering, independent download aggregation, and explicit offline availability | Fork follow-up to F07 and the completed-download Library | iOS, iPadOS, and Android shared source | In testing | No equivalent found at the reviewed base | [PR #32](https://github.com/tinyKyuu/NuvioMobile/pull/32); primary views are All titles, Downloads, and conditional Cloud files. All titles is a deduplicated union of explicit local membership, the selected connected Trakt or Simkl library, and playable downloads; downloading does not change local or tracking membership, while deleting an unsaved download removes it from All titles. The explicit Watch status filter precedes the selected sort, and partly watched series remain Unwatched until existing completion state marks the series complete. Offline title cards distinguish downloaded media from internet-required entries, and internet-required cards stay in Library with an explanation instead of opening blank details. Downloaded, internet-required, watched, Manage-menu, and active-selection poster overlays use one shared badge component with regular and large width-driven sizes; accent badges no longer add a dark outline, and large vertical Library cards add concise media-type and year metadata. Episode artwork groups watched and availability states on one compact status rail. Offline series details offer a session-local Downloaded-only filter when it can remove unavailable episodes and empty seasons. Download management replaces the root dock, Clear exits management, overflow affordances are horizontal, and destructive actions use the crimson treatment. Wide Library checks verified inline source choices, trailing Manage or Done plus download/view actions, watched choices, and downloaded badges; compact and stacked presentations, canonical deduplication, provider visibility, watched projection, and offline navigation are covered by host tests. The latest complete 1,143-test Android-host suite, both Android debug variants, Kotlin/Native iOS simulator test compilation, unsigned simulator build, and signed physical-iPad build passed. The build was installed in place and launched without clearing app data; interactive acceptance remains pending. |
