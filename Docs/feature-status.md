...
| Ordered add-on and catalog recovery after connectivity returns, with a bounded per-profile manifest cache | Fork state machine and cache design, with delayed-state presentation adapted from official Nuvio commit [`085e8dc6`](https://github.com/NuvioMedia/NuvioMobile/commit/085e8dc6aaf5072541130be852a782998fc3dbad) | iOS, iPadOS, and Android shared source | In testing | Adapted; official upstream has delayed loading/error states but no equivalent central recovery coordinator or bounded, versioned per-profile manifest cache at the reviewed tip | [PR #26](https://github.com/tinyKyuu/NuvioMobile/pull/26) and [F07 review evidence](../F07-REVIEW.md); tested implementation `7ecc0806`; 1,042 Android-host tests, 63 focused tests, both Android debug variants, Kotlin/Native test compilation, and the unsigned full-distribution Xcode simulator build passed. Regressions cover fresh-provider readiness while missing/stale providers remain held, independent Home/Search publication, Discover selection, successful empty Search replacement versus failed-request retention, one-time interrupted Details retry, title/profile ownership, fresh-probe Retry, manual refresh, repeated outages, and concurrent cache persistence. Tests inject transport, metadata/snapshot callbacks, or storage; platform snapshot persistence is not newly verified. Final-build controlled Android checks verified fresh A Home/Discover while B remained held, automatic Details retry after global completion, and mixed/all-empty Search replacement through manual recovery. Foreground-triggered Search attempts were inconclusive and are not counted. The rebuilt iPhone app rendered Home. Interactive iOS recovery, physical-device Wi-Fi transitions, and release-candidate offline playback remain deferred. |
