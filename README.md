...
| Tablet root navigation uses the shared Compose dock with a bounded, theme-aware iPad presentation, stable labels, and an animated glass selection capsule with selected-item emphasis. Android tablets retain Adaptive, Expanded, Compact, and Classic choices; measured width fallback drops Settings first, then all labels only when required. Keyboard and Download activity transitions restore the dock reliably, with iPad keyboard dismissal returning it directly to the physical bottom safe area. | App navigation | iPadOS and Android tablets | [In testing](https://github.com/tinyKyuu/NuvioMobile/pull/34) |
