...
| Bounded manual reconnect sessions and responsive root-header connection controls on Home, Search, and Library | Fork follow-up to F07 | iOS, iPadOS, and Android shared source | In testing | No equivalent found at the reviewed base | [PR #32](https://github.com/tinyKyuu/NuvioMobile/pull/32) at independently tested code commit `fbc3dd66305f945a87f6c10ce5f0d7e884d90888`; one press probes immediately, makes at most three attempts with 2-second and 4-second backoff, and stops within 15 seconds. Repeated presses and path events coalesce, backgrounding or profile replacement cancels the session, and local-only Home remains until catalog recovery completes. The connection control is limited to root Home, Search, and Library headers, stays trailing and inline until the width is genuinely narrow, uses a trailing Wi-Fi-off graphic, and drops that graphic before stacking. Library shares its source choices and context actions on one row from compact-tablet widths upward, while Manage Downloads and Done use their natural widths. The tablet dock switches to an icon-only presentation when a narrow window cannot fit all labels. 13 focused responsive-layout tests and 1,131 complete Android-host tests, both Android debug variants, Kotlin/Native iOS simulator test compilation, iPhone and iPad offline simulator checks, and a signed physical-iPad build passed. The iPad simulator verified the trailing connection graphic plus Saved, Downloads, Manage Downloads, and Done alignment; the physical iPad verified that the four-item compact dock fits a narrow Stage Manager window without clipping. |
