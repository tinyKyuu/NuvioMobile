...
| Bounded manual reconnect sessions and responsive root-header connection controls on Home, Search, and Library | Fork follow-up to F07 | iOS, iPadOS, and Android shared source | In testing | No equivalent found at the reviewed base | [PR #32](https://github.com/tinyKyuu/NuvioMobile/pull/32) at independently tested code commit `c934b68f1d71b673829086cc113be7868b750ba0`; one press probes immediately, makes at most three attempts with 2-second and 4-second backoff, and stops within 15 seconds. Repeated presses and path events coalesce, backgrounding or profile replacement cancels the session, and local-only Home remains until catalog recovery completes. The connection control is limited to root Home, Search, and Library headers and stacks below the title at narrow widths; Library keeps source choices and context actions on separate lower rows. 218 focused and 1,129 complete Android-host tests, both Android debug variants, Kotlin/Native iOS simulator test compilation, an unsigned iOS simulator build, and iPhone plus iPad simulator checks passed. A signed build installed in place and launched on the physical iPad. Landscape Library and portrait narrow-window Home checks passed for the F07 headers; the existing tablet dock clips its Settings item at the tested narrow Stage Manager width and remains outside this PR's scope. |
