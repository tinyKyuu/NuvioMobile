...
| Ordered add-on and catalog recovery after connectivity returns, with a bounded per-profile manifest cache | Fork state machine and cache design, with delayed-state presentation adapted from official Nuvio commit [`085e8dc6`](https://github.com/NuvioMedia/NuvioMobile/commit/085e8dc6aaf5072541130be852a782998fc3dbad) | iOS, iPadOS, and Android shared source; Android recovery flow tested | In testing | Adapted; official upstream has delayed loading/error states but no equivalent central recovery coordinator or bounded, versioned per-profile manifest cache at the reviewed tip | [PR #26](https://github.com/tinyKyuu/NuvioMobile/pull/26) and [F07 review evidence](../F07-REVIEW.md); implementation head `1530d20b`; 1,006 Android-host tests, both Android debug variants, Kotlin/Native test compilation, and the full-distribution Xcode simulator build passed; an integration test drives production recovery ordering and actual `HomeRepository` batching, proving the healthy row publishes before the stale provider's catalog starts and that final reconciliation still includes the stale provider; narrower deterministic coverage verifies empty-cache manifest selection, replacement of a suspended background request by reconnect recovery, coalescing, profile invalidation, and Search query replacement, but does not model a complete cold offline launch and reconnect; Android emulator exercised offline-to-online recovery with one healthy and one unavailable add-on during the initial pass; the rebuilt iPhone simulator app cold-launched, but an interactive iOS network transition and physical iPad recovery remain required before release |
