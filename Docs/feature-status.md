...
| Trakt and Simkl tracking-build readiness and OAuth callback hardening | Fork follow-up to PR #32 tracking-source validation | iOS, iPadOS, and Android shared source | In review | No equivalent found at the reviewed base | This PR keeps provider configuration private, supports ignored local properties or environment variables, adds a non-secret release verification task, and disables sign-in with a user-facing build-availability message when provider configuration is absent. Trakt callbacks now require the exact pending OAuth state for success and provider-error responses; Simkl retains PKCE and shares the supported-redirect readiness check. Live provider-account acceptance remains pending until separately registered Trakt and Simkl application credentials are supplied through private build configuration. |
