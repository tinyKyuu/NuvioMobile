...
| Trakt and Simkl tracking-build readiness and OAuth callback hardening | Fork follow-up to PR #32 tracking-source validation | iOS, iPadOS, and Android shared source | In review | No equivalent found at the reviewed base | This PR keeps provider configuration private, supports ignored local properties or environment variables, adds a non-secret release verification task, and disables sign-in with a user-facing build-availability message when provider configuration is absent. Simkl appears first as the configured tracking option; an unconfigured Trakt card states that it is unavailable instead of presenting a dead Connect action. Disconnected and pending browser-authorization states share one stable Connect action, so leaving the browser does not resize the card; repeated taps resume a still-valid attempt, while internal authorization state expires after ten minutes. Trakt callbacks require the exact pending OAuth state for success and provider-error responses. Simkl is migrated to AUTH V2 as a public PKCE client with a fork-specific callback, exact state and issuer validation, `media:read media:write` scope enforcement, platform-secure access and refresh token storage, proactive and one-time `401` refresh, and best-effort grant revocation on disconnect. No provider identifier or token is committed. The current head passes the complete 1,173-test Android-host suite, both Android debug variants, Kotlin/Native iOS simulator test compilation, a full unsigned simulator build, and the tracking configuration verifier with non-secret placeholders. The tracking-card refinement also passes its focused Android-host tests and iOS compilation, and a signed build with private Simkl configuration was installed and launched on the physical iPad. Live account approval, callback return, initial sync, refresh, and disconnect acceptance remain pending a personal test account. |
