...
| Library-first completed-download catalog and manager, current-transfer bulk actions in Download activity, separate policy settings, exact episode removal, and fail-safe partial cleanup | Fork addition | iOS, iPadOS, and Android shared source | In review | No equivalent found at the reviewed base | [PR #27](https://github.com/tinyKyuu/NuvioMobile/pull/27) and [F08A review evidence](../F08-REVIEW.md); no schema migration or version change. A failed completed target keeps playable media and its catalog record. A canceled current transfer is removed even if ancillary request or partial cleanup reports a warning. Activity select-all is limited to displayed current transfers, and Library owns completed selection. F08B visual hardening and F08C artwork repair remain deferred. |
...
| Automatic foreground network restoration with ordered add-on/catalog recovery, local-only offline Home, and a bounded per-profile manifest cache | Fork state machine, event-driven connectivity observation, and cache hardening; delayed-state presentation adapted from official Nuvio commit [`085e8dc6`](https://github.com/NuvioMedia/NuvioMobile/commit/085e8dc6aaf5072541130be852a782998fc3dbad), with the six-hour freshness intent retained from official draft [PR #1897](https://github.com/NuvioMedia/NuvioMobile/pull/1897) | iOS, iPadOS, and Android shared source | In testing | Adapted; official upstream supplies delayed loading/error presentation and a draft cache donor, but no equivalent event-driven path observer, central recovery coordinator, or bounded/versioned stale-while-revalidate cache at the reviewed tip | [PR #26](https://github.com/tinyKyuu/NuvioMobile/pull/26) and [F07 review evidence](../F07-REVIEW.md); tested implementation `a0e3eaf9d09983e056a5a9e267ed2e39ea77caee`. Android recovery wake-ups require a validated path, default-path loss is authoritative, and HTTP supplies the final condition. Forced requests reserve one coalesced pending probe. A failed matching force-refresh probe clears its generation and force-all intent together, while a successful matching probe consumes force-all once. Typed manifest events keep cached admission cumulative but scope stale changes, missing recovery, and force-all publication to one provider; automatic Unchanged results only renew freshness. Manual force-all publishes each successful provider as it settles and still performs the final full pass. Add-on cards force-refresh only their selected URL. One app-shell reset state owns Home transition production and consumption across hidden tabs and activity restoration. Evidence: six primary red regressions, the runtime-discovered loss-edge red, and the failed force-intent lifecycle red; 125 focused and 1,072 complete Android-host tests; Full and Play Store debug APKs; Kotlin/Native test compilation; unsigned Full iOS simulator build; Android foreground airplane loss, failed manual Reconnect, automatic restoration, live Reconnecting/Restoring states, and dismissal after completion. Version remains `0.4.12` build `122`. Deferred: interactive iOS path switching, physical iPad/Android Wi-Fi transitions, light/dark/AMOLED styling, physical hidden-tab scroll interaction, Downloads, local Continue Watching, offline Details/local playback, and signed/TestFlight release-candidate checks. |
