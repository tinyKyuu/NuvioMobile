...
| Browse and manage completed movies, shows, seasons, and episodes from Library > Downloads; view current transfers with collapsible download-policy controls on one Downloads page | Library, Downloads, and Settings | iOS, iPadOS, and Android shared source | [Merged](https://github.com/tinyKyuu/NuvioMobile/pull/27) with an [in-review refinement](https://github.com/tinyKyuu/NuvioMobile/pull/32) |
...
| Header-integrated connection status keeps one finite Reconnect session trailing on Home, Search, Library, and offline Details, uses a quiet idle action with a stronger restoring state, removes its Wi-Fi-off graphic before stacking, and includes a transient offline test switch | Home, Search, Library, Details, and Advanced settings | Shared mobile source; iPhone and iPad simulators tested | [In review](https://github.com/tinyKyuu/NuvioMobile/pull/32) |
...
| Tablet root navigation uses one measured floating dock with a shared selected indicator: labels sit below icons when they fit, Settings drops its label first, and all labels disappear only when required | App navigation | iPadOS and Android tablets | [In review](https://github.com/tinyKyuu/NuvioMobile/pull/32) |
...
| Adaptive All titles, Downloads, and eligible Cloud files views share measured controls; All titles deduplicates local saves, the selected tracking library, and playable downloads without changing membership, watch status is filterable, and poster status and overflow controls share one reusable badge treatment | Library | iOS, iPadOS, and Android shared source | [In review](https://github.com/tinyKyuu/NuvioMobile/pull/32) |
| Downloaded episodes remain playable from the normal series details screen and expose an exact-episode removal shortcut; shared watched, downloaded, and internet-required badges replace repeated labels, while offline Details offers a compact `Availability: All / Downloaded` filter and shared Reconnect action | Details | iOS, iPadOS, and Android shared source | [In review](https://github.com/tinyKyuu/NuvioMobile/pull/32) |
