...
| Bounded manual reconnect sessions, measured root-header controls, a measured tablet dock, and transient offline simulation | Fork follow-up to F07 | iOS, iPadOS, and Android shared source | In testing | No equivalent found at the reviewed base | [PR #32](https://github.com/tinyKyuu/NuvioMobile/pull/32); one press probes immediately, makes at most three attempts with 2-second and 4-second backoff, and stops within 15 seconds. Repeated presses and path events coalesce, backgrounding or profile replacement cancels the session, and local-only Home remains until catalog recovery completes. Reconnect stays trailing, places the Wi-Fi-off graphic after its text, drops that graphic before stacking, and is governed by measured fit instead of a fixed width breakpoint. The tablet dock keeps Home, Search, and Library labels while their measured candidate fits, keeps Settings avatar-only, and otherwise falls back to icons. Advanced settings can simulate offline mode without changing device Wi-Fi; Reconnect clears the transient simulation before normal recovery. The complete 1,137-test Android-host suite, both Android debug variants, Kotlin/Native iOS simulator test compilation, and the unsigned iOS simulator build passed. iPhone and iPad simulator checks covered the trailing graphic, labeled dock, avatar-only Settings, simulation toggle, and reconnect reset; the earlier header/dock checkpoint was also installed and reviewed on a physical iPad. |
| Adaptive Library title views, watched filtering, local download saving, and explicit offline availability | Fork follow-up to F07 and the completed-download Library | iOS, iPadOS, and Android shared source | In testing | No equivalent found at the reviewed base | [PR #32](https://github.com/tinyKyuu/NuvioMobile/pull/32); All titles is the deduplicated union of Saved and playable Downloads, Cloud files appears only for a compatible connected provider, and All, Unwatched, or Watched filtering precedes the selected sort. Partly watched series remain Unwatched until existing completion state marks the series complete. Successful new downloads save the title only to Nuvio's local library and never mutate Trakt or Simkl; deleting media does not unsave it. Offline title cards distinguish downloaded media from internet-required entries, and internet-required cards stay in Library with an explanation instead of opening blank details. Wide Library checks verified inline source choices, trailing Manage or Done plus download/view actions, watched choices, and downloaded badges; compact and stacked presentations, canonical deduplication, provider visibility, watched projection, and offline navigation are covered by host tests. Current signed physical-iPad acceptance remains pending. |
