...
| Automatic foreground network restoration with ordered add-on/catalog recovery, local-only offline Home, and a bounded per-profile manifest cache | Fork state machine, event-driven connectivity observation, and cache hardening; delayed-state presentation adapted from official Nuvio commit [`085e8dc6`](https://github.com/NuvioMedia/NuvioMobile/commit/085e8dc6aaf5072541130be852a782998fc3dbad), with the six-hour freshness intent retained from official draft [PR #1897](https://github.com/NuvioMedia/NuvioMobile/pull/1897) | iOS, iPadOS, and Android shared source | In testing | Adapted; official upstream supplies delayed loading/error presentation and a draft cache donor, but no equivalent event-driven path observer, central recovery coordinator, or bounded/versioned stale-while-revalidate cache at the reviewed tip | [PR #26](https://github.com/tinyKyuu/NuvioMobile/pull/26) and [F07 review evidence](../F07-REVIEW.md); tested implementation `d84afa64307e12f0c7d88d5aa14a8d06146284ca`. Android uses `ConnectivityManager.NetworkCallback`, iOS uses `NWPathMonitor`, and HTTP remains authoritative. Valid stale manifests stay usable while unchanged refreshes renew timestamps, changed refreshes reconcile only their provider, and failures retain cached content. The root states are Offline/Reconnect, Reconnecting, Restoring content, actionable global failure, and completed/hidden; `Back online` waits for settled catalog reconciliation. Home swaps remote hero/rows and local Downloaded content atomically and consumes one reset-to-top generation per confirmed direction, including after a hidden-tab transition. Evidence: 100 focused and 1,063 complete Android-host tests; Full and Play Store debug APKs; Kotlin/Native test compilation; unsigned Full iOS simulator build; final Android foreground airplane loss, failed manual Reconnect, automatic path-callback restoration, and coordinator completion; final iOS install/cold launch. Version remains `0.4.12` build `122`. Deferred: interactive iOS path switching, physical iPad/Android Wi-Fi transitions, light/dark/AMOLED styling, hidden-tab scroll interaction, Downloads, local Continue Watching, offline Details/local playback, and signed/TestFlight release-candidate checks. |
