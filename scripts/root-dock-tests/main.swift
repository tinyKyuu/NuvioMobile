...
check(landscape.isVertical, "A wide landscape window must use a side dock")
...
check(portrait.size.width == 400 && portrait.size.height == 60, "Bottom pill stays compact on a wide iPad")
check(landscape.itemSize == CGSize(width: 64, height: 64), "Side selector uses compact concentric slots")
check(portrait.reservedHeight == 76, "Bottom scroll clearance matches the smaller dock")
