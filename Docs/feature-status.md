...
| Bounded manual reconnect sessions, measured root-header controls, a measured tablet dock, and transient offline simulation | Fork follow-up to F07 | iOS, iPadOS, and Android shared source | In testing | No equivalent found at the reviewed base | [PR #32](https://github.com/tinyKyuu/NuvioMobile/pull/32); one press probes immediately, makes at most three attempts with 2-second and 4-second backoff, and stops within 15 seconds. Repeated presses and path events coalesce, backgrounding or profile replacement cancels the session, and local-only Home remains until catalog recovery completes. Reconnect stays trailing on Home, Search, and Library, reserves stable widths across Reconnect, Restoring, and failure states, uses a low-prominence idle treatment, and strengthens its pressed and restoring feedback. The tablet dock remains one pill-shaped surface at every width: destinations share one selected indicator, labels sit below icons in the full presentation, Settings drops its label first, and all labels disappear only when the measured width requires it. Root navigation now yields its space while the software keyboard or download selection is active. Nested wide-tablet Settings pages retain a local parent back action even when native iOS navigation is enabled. Advanced settings can simulate offline mode without changing device Wi-Fi; Reconnect clears the transient simulation before normal recovery. The latest complete 1,149-test Android-host suite, both Android debug variants, Kotlin/Native iOS simulator test compilation, unsigned simulator build, and signed physical-iPad build passed. The refreshed build was installed in place and launched without clearing app data; interactive acceptance remains pending. |
| Adaptive Library title views, watched filtering, independent download aggregation, and explicit offline availability | Fork follow-up to F07 and the completed-download Library | iOS, iPadOS, and Android shared source | In testing | No equivalent found at the reviewed base | [PR #32](https://github.com/tinyKyuu/NuvioMobile/pull/32); primary views are All titles, Downloads, and conditional Cloud files. All titles is a deduplicated union of explicit local membership, the selected connected Trakt or Simkl library, and playable downloads; downloading does not change local or tracking membership, while deleting an unsaved download removes it from All titles. The shared filter row keeps stable identity while switching All titles and Downloads, so its controls and scroll position do not fade out and reload. The explicit Watch status filter precedes the selected sort, while a Genre filter lists only metadata present in the current scope and temporarily falls back to All Genres when a prior selection is unavailable; partly watched series remain Unwatched until existing completion state marks the series complete. Offline title cards distinguish downloaded media from internet-required entries, and internet-required cards stay in Library with an explanation instead of opening blank details. Downloaded, internet-required, watched, and overflow-menu poster overlays use one shared badge component with regular and large width-driven sizes; selection instead uses an accent outline and full-poster wash so it does not compete with watched state. One episode status is circular while combined statuses share a pill, and large vertical Library cards add concise media-type and year metadata. Offline series Details keeps its season heading stable and places a short session-local `All | Downloaded` selector beside it; filtering removes empty seasons without hiding the remaining season chip, the first season label aligns with the heading, and the animated season snapshot remains valid while outgoing content leaves. Download selection routes Select, Deselect, Close, and Remove through one shared compact action component with normal-weight body text and a medium Close icon. Header and collapsed manager actions center a 32 dp filled control in a 44 dp touch target without an outline; press feedback stays inside the visible control, the active header Close control uses the same selected accent treatment as the active Library view, and the compact manager uses visually equal gaps between Deselect, Remove, and Close. The expanded chooser retains an outline drawn by the same clipped surface as its fill and omits its redundant empty-selection subtitle. Destructive controls keep the same geometry in crimson. The compact manager remains on one row, the large completed-download chooser has no empty footer, a fully successful removal exits selection, and partial failures remain selected for recovery. The Downloads page keeps compact Download settings above current transfers, states its two-transfer concurrency rule, and does not duplicate completed-library management. Canonical deduplication, provider visibility, watched and genre projection, offline navigation, season-filter transitions, narrow manager presentation, Settings parent navigation, and selection state are covered by host tests. The complete 1,152-test Android-host suite, both Android debug variants, Kotlin/Native iOS simulator test compilation, and signed physical-iPad build pass. The refreshed build is installed and launched without clearing app data; interactive acceptance remains pending. |
